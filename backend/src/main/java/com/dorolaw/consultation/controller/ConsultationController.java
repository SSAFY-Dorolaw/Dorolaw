...
public class ConsultationController {
    ...
    public ResponseEntity<ConsultationBookResponseDto> bookConsultation(
        ...
        alarmService.sendConsultationConfirmAlarms(response.getLawyer().getLawyerId(),response.getClient().getClientId(),response.getScheduledDate(), response.getRequestId());
        return ResponseEntity.ok(response);
    }
